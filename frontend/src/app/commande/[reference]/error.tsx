/**
 * Route UI: app/commande/[reference]/error
 *
 * Rôle du fichier :
 *   Error Boundary de la page de confirmation de commande. Affiché si
 *   la page lève une exception (rare ici : la majorité des erreurs sont
 *   converties en `null` puis `notFound()` dans `page.tsx`).
 *
 *   Ne couvre PAS le cas « commande introuvable » (qui passe par 404).
 *
 * Prérequis :
 *   - DOIT être un Client Component (Error Boundary React).
 *   - Props standard Next : `error` et `reset`.
 */

"use client";

import { useEffect } from "react";
import Link from "next/link";

type Props = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function CommandeError({ error, reset }: Props) {
  useEffect(() => {
    console.error("Erreur de chargement de la commande :", error);
  }, [error]);

  return (
    <section className="container py-5">
      <div className="text-center mx-auto" style={{ maxWidth: "520px" }}>
        <i
          className="bi bi-wifi-off text-warning"
          style={{ fontSize: "3rem" }}
          aria-hidden="true"
        ></i>
        <h1 className="h3 fw-bold mt-3">
          Impossible d&apos;afficher cette commande
        </h1>
        <p className="text-secondary">
          Une erreur est survenue pendant le chargement de votre commande.
          Vérifiez votre connexion, puis réessayez.
        </p>
        <div className="d-flex flex-wrap justify-content-center gap-2 mt-4">
          <button type="button" className="btn btn-dark" onClick={() => reset()}>
            <i className="bi bi-arrow-clockwise me-2" aria-hidden="true"></i>
            Réessayer
          </button>
          <Link href="/commandes" className="btn btn-outline-dark">
            Voir mes commandes
          </Link>
        </div>
      </div>
    </section>
  );
}
